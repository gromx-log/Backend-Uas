    ...
    <div class="bg-white rounded-2xl shadow-md p-6 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500">{{ $following->total() }} Following</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-md">
        @forelse($following as $followedUser)
            <div class="flex items-center justify-between p-6 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                <div class="flex items-center gap-4">
                    <!-- Avatar -->
                    <div class="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center text-lg font-bold text-gray-600">
                        {{ strtoupper(substr($followedUser->username, 0, 2)) }}
                    </div>
                    ...
                    <div class="flex-1">
                        ...
                        @if($followedUser->bio)
                            <p class="text-gray-700 text-sm mt-1">{{ Str::limit($followedUser->bio, 100) }}</p>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="p-12 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-500 mb-2">Not following anyone</h3>
                <p class="text-gray-400">{{ $user->username }} isn't following anyone yet.</p>
            </div>
        @endforelse
    </div>
